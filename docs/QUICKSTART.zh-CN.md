...
Rust 2024 edition，最低版本 1.89。不需要起服务，不需要下载数据——示例的行情由
种子自己生成。
...
上面所有内容跑的都是录下来的数据。这一节是同一个循环,只是事件由交易所
提供而不是由文件提供——这一步没法靠读代码验证,因为回测和实盘的区别
**完全在于订单离开之后发生了什么**。

你需要交易所的 testnet 凭据。**testnet。** 这里没有任何一步要求你承担
风险,而且这个示例直接拒绝 `--live`。
...
如果它发现旁边有一个没人告诉过它的仓位,它会拒绝启动。这不是为谨慎而
谨慎:一个自作主张接管了来历不明仓位的进程,没有任何办法分辨那是自己
上次留下的还是别人的,而它做的第一件事就是去管理它。`--adopt-existing`
表示你已经确认过了。

### 为什么是网格

它是"做空波动率且没有止损",而这恰好是保证金模型存在的意义所在:每一格
在区间破掉之前都是赚的,区间一破,累积的仓位就站在趋势的错误一侧,而且
体量大于任何单次决策批准过的数字。长时间跑它,能压到这个框架里最难用
别的方式压到的那部分。

**它不是推荐。** 它是最值得看着它失败的那个策略。

### 该看什么

运行时会先打一条 banner,写明它将强制执行的限额及其版本,然后每单一行。
有三样东西值得读、而不是扫过去:

- **`placed` 和 `refused` 是两个不同的数。** 汇总行两个都报,它们的比值
  正是回测给不出的东西:模拟撮合对每一次提交都会答复,所以"要了什么"和
  "被接受了什么"只在那里重合。
- **出现 `UNRESOLVED` 行,意味着账户不一定在汇总所说的位置上。** 它单独
  打印,而且只在真发生时打印,因为它是唯一一种既不是"是"也不是"否"的结果。
  这种订单不会被重发——重发是唯一一个能把"也许有一单"变成"确定有两单"的
  动作。
- **被拒是正常的,而且故意很吵。** 风控限额触发说明闸门在工作。网格自己
  的格数上限和它运行的限额都被刻意设小,好让这件事在半小时内发生,而不是
  永远不发生。
- **收尾时的 metrics 和 alerts。** 这里的 alert 是**判断**,不是通知——
  没有任何东西会被发出去。把它接到真会发通知的东西上属于部署,这是本框架
  刻意不做的事。

### 如果你想让它走 `oq-trade`

`oq-trade` 有 `observe`(不发单)和 `probe`(连通性诊断,不是策略)。两者
都不是策略运行器,这是有意的:本仓库的策略住在 `oq-examples` 里,那个 crate
不发布,而一个要发布的 `oq-live` 不能依赖未发布的 crate。写你自己的看第 5
节,`grid_live.rs` 一共约三十行,可以直接抄。
...
`series` 和 `crash_series` 是**夹具,不是仿真**,而 `oq_stats::StylizedFacts`
把这个差别测出来,不留给人去假定。对照那些在几乎每个流动市场都成立的性质:

| | calm | trending | crash |
|---|---|---|---|
| 收益率无自相关 | 成立 | 成立 | **不成立,ρ(1) = 0.54** |
| 重尾 | 不成立 | 不成立 | 不成立 |
| 波动率聚集 | 不成立 | 不成立 | 成立 |
| 聚合高斯性 | 成立 | 不成立 | 成立 |

由此有两条推论,都会改变上面那些数字该怎么读。

**没有一个有重尾。** 超额峰度分别是 0.03、0.07、−0.05,而真实永续在**几十**的
量级。爆仓是尾部事件,所以一个没有尾部的夹具只会在被安排的地方产生爆仓——
因此这些示例报出的"有保证金 vs 无保证金"差距,是真实差距的**下界**,而不是
对它的估计。

**crash 夹具的收益率强自相关。** 一段持续单向的行情意味着连续数百个观测的
收益率同号,单滞后规则就能预测,而真实市场不提供这个。正是这种可预测性使它
成为一个可用的夹具——爆仓发生在被安排的位置——也正因如此,从它得出的策略
结果是关于这条序列的陈述,而不是关于交易的。

这些测量钉在 `crates/oq-examples/tests/stylized.rs` 里,改生成器的人当场会知道。
