...
| Core | **Sharding by instrument** | **Blocked on a prior decision**, not merely unwritten (`shard` appears nowhere; FR-CORE-6). A shard is one instrument's kernel, and `State` already holds exactly one instrument — but it also holds the balance and the position, and an account trading two instruments shares both. **The account is now separated from the instrument.** `State` holds a list of `Holding`s — each one instrument's matcher, position and margin table, with nothing in it shared and no way to reach the account from it, which is the direction FR-CORE-6 rules out. Equity and maintenance are summed across them, so two instruments share one balance and one margin requirement. **The strategy surface names one now:** `Intent::Limit` and `Intent::Market` carry an `InstrumentId`, `Context` says which instrument a callback is about and builds intents for it, and an order for an instrument a run does not hold is refused and reported through `on_placed` rather than sent to the only holding there is. **The input side names one too:** `Event::Tick` carries an optional instrument under `kind::TICK_ON`, with the older 64-byte kind still read as "the account's only one" — which is what it meant. An unnamed observation is refused once the account holds two, rather than guessed at. So both directions route, and what remains is the mechanical part: one kernel per shard, one thread each, and a stream that fans out to them. Both are schema and API decisions rather than missing code, and the second is constrained by FR-CORE-6's rule that shards interact only through the sequenced event stream. Sharding it as it stands gives each shard its own balance, which no multi-instrument strategy can use and no single-instrument one needs |
