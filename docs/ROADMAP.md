...
| L2 | **An archive converted into a run's input** | **Not built** — `oq-ingest` produces a tick file, which by construction cannot carry a book. Feeding L2 from an archive today means the caller writing the loop that interleaves the two streams; `book_tiers` shows the shape with a generated book. Until it is a command, the tier is reachable in principle and hand-assembled in practice |
