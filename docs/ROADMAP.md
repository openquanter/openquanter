...
| L2 | **An archive converted into a run's input** | Built — `fold_into_observations` interleaves ticks and depth in the order the venue produced them, and `oq-tiers` is the command: one strategy, one archive, two tiers. On one hour of captured BTCUSDT (134,611 depth updates, none refused) a bid resting at the touch filled **1289 times at L0 and 340 at L2** — 73.6% of the backtest's fills are trades the queue never reached. The book is bootstrapped from the first update rather than a REST snapshot, which the archive does not hold, so early queues read shorter than they were |
| L2 | **Depth in the journal** | **Not built**, and it is the one place replay is not faithful: an L2 run's journal reproduces its orders but not the book they matched against, so its fills differ. An `Event` is a fixed-size record and a depth update is a variable-length list of levels, so this is a format change with a version behind it rather than a field. Until then an L2 run's evidence is its archive **plus** its journal |
